...
func NewController(
	...
	worker, err := worker.New(dbConn, queueName, worker.WithPollInterval(workerPollInterval))
	if err != nil {
...
func (c *Controller) registerTasks() {
	worker.ForwardToAsyncTask(c.worker, c.usecaseContainer.CleanExpiredSessions())
}
...
func (c *Controller) registerSchedules() error {
	...
	err := c.scheduler.RegisterSchedules(
		...
		scheduler.Schedule{
			...
			OperationID: c.usecaseContainer.CleanExpiredSessions().OperationID(),
		},
	)
