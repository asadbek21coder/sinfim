...
func TestEnableUser_Success(t *testing.T) {
	...
	// WHEN
	resp := trigger.UserAction(t).POST("/api/v1/auth/enable-user").
	...
	assert.True(t, updated.IsActive, "user should be enabled")
...
func TestEnableUser_AuthFailures(t *testing.T) {
	tests := []struct {
		name       string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// WHEN
			req := trigger.UserAction(t).POST("/api/v1/auth/enable-user").
